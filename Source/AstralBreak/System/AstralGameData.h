...
public:
	...
	/** 오의 수급 GE (SetByCaller.UltGain → UltGain meta) — 어빌리티 적중/패링 보상 + 피격 수급(PlayerState) 공유 */
	UPROPERTY(EditDefaultsOnly, Category = "Default Gameplay Effects", meta = (DisplayName = "UltGain Gameplay Effect (SetByCaller)"))
	TSoftClassPtr<UGameplayEffect> UltGainGameplayEffect_SetByCaller;

	/** 표식 수급 GE (SetByCaller.MarkGain → MarkGain meta) */
	UPROPERTY(EditDefaultsOnly, Category = "Default Gameplay Effects", meta = (DisplayName = "MarkGain Gameplay Effect (SetByCaller)"))
	TSoftClassPtr<UGameplayEffect> MarkGainGameplayEffect_SetByCaller;

	/** 스태미나 가변 소모 GE (SetByCaller.StaminaDrain, 음수 주입) — 가드 피격 등 */
	UPROPERTY(EditDefaultsOnly, Category = "Default Gameplay Effects", meta = (DisplayName = "StaminaDrain Gameplay Effect (SetByCaller)"))
	TSoftClassPtr<UGameplayEffect> StaminaDrainGameplayEffect_SetByCaller;
